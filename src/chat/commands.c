...
/*
 * FIX M-6: after updating the session nickname, also update the TUI's
 *          internal g_my_nick copy via tui_set_nick() so the input bar
 *          stays in sync and /me reports the correct name.
 * FIX L-6: notify the discovery beacon thread of the new nick so that
 *          peers who rescan see the updated name immediately.
 */
static CmdResult handle_nick(const char *args, char *nickname, Session *s) {
    if (!args || args[0] == '\0') {
        tui_status("Usage: /nick <name>");
        return CMD_OK;
    }
    ...
    /* FIX M-6: keep TUI input bar in sync. */
    tui_set_nick(nickname);

    /* FIX L-6: keep beacon in sync. */
    discovery_set_nick(nickname);
...
static CmdResult handle_hideotherpanels(const char *args, char *nickname,
                                        Session *s) {
    (void)args; (void)nickname; (void)s;
    tui_toggle_panels();
    return CMD_OK;
